...
def search():
    ...
    # validate screen_name less @ or redirect to index
    screen_name = request.args.get("screen_name", "").lstrip("@")
    if not screen_name:
        return redirect(url_for("index"))

    # get tweets from screen_name
    ...
    # determine percentage for pie chart
    total_score = 0
    positive, negative, neutral = 0.0, 0.0, 0.0
        
    # iterate through each tweet and print out them in color up to 100 tweets
    if tweets != None:
        for i in range(100):
            if i >= len(tweets):
                break
